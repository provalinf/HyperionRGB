...
void WrapperWebconfig::handleRoot(void) {
	...
	clearHelperVars();
	String message = htmlTemplate("HyperionRGB Configuration v2.0", WrapperWebconfig::config());
...
void WrapperWebconfig::changeConfig(void) {
	...
	for (uint8_t i = 0; i < _server->args(); i++) {
		...
		if (argName == "wifi-ssid" && argValue.length() < sizeof(cfg->wifi.ssid)) {
			...
			byte ip[4];
			if (strcmp(argValue.c_str(), "") == 0) argValue = "0.0.0.0";
		...
		} else if (argName == "led-type") {
			cfg->led.ledType = getSelectedEntry<uint8_t>(argValue, _typesLed);
		} else if (argName == "led-number") {
			cfg->led.nbLed = argValue.toInt();
...
String WrapperWebconfig::textTemplate(String label, String tooltip, String id, String text, String placeholder = "",
									  int maxLen = 524288) {
	String html = "";

	html += "<input id=\"" + id + "\" name=\"" + id + "\" type=\"text\" maxlength=\"" + maxLen + "\" placeholder=\"" +
			placeholder + "\" class=\"form-control input-md\" value=\"" + text + "\"/>";
...
String WrapperWebconfig::checkboxTemplate(String label, String tooltip, String id, boolean isChecked) {
	...
	html += "<input id=\"" + id + "\" name=\"" + id + "\" type=\"checkbox\" value=\"" + id + "\" class=\"input-md\"" +
			checked + "/>";
	html += "</label>";
...
String
WrapperWebconfig::selectTemplate(String label, String tooltip, String id, LinkedList<SelectEntryBase *> *entries) {
	String html = "";
...
String WrapperWebconfig::config(void) {
	...
	html += "<legend>HyperionRGB Configuration v2.0</legend>";
	...
	groupContent += textTemplate("WiFi Password", "", "wifi-password", "", passwordPlaceholder,
	...
	groupContent += textTemplate("Module Hostname", "", "wifi-hostname", escape(cfg->wifi.hostname), "HyperionRGB-TV",
								 sizeof(cfg->wifi.hostname) - 1);
	...
	if (wifiReady) {
		...
		_typesLed = new LinkedList<SelectEntryBase *>();
		getLedType(cfg->led.ledType, _typesLed);
		groupContent += selectTemplate("Led Type", "", "led-type", _typesLed);
		groupContent += textTemplate("Led number", "", "led-number", escape(cfg->led.nbLed), "100", 3);
		clearLinkedList(_typesLed);
		...
		groupContent += checkboxTemplate("Autoswitch to Hyperion_UDP/Idle Mode",
										 "Automatically switch to Hyperion_UDP when UDP Data arriving and switch back to idle mode after timeout",
										 "led-autoswitch", cfg->led.autoswitch);
		groupContent += textTemplate("Timeout Fallback in MS",
									 "Switches back to Idle Mode after x milliseconds when no UDP data is arriving",
									 "led-timeoutMs", escape(cfg->led.timeoutMs), "5000", 10);
	...
	groupContent += "<div class=\"form-group\">";
	groupContent += "<label class=\"col-md-4 control-label\" for=\"resetConfig\"></label>";
	groupContent += "<div class=\"col-md-4\">";
	groupContent += "<button id=\"resetConfig\" name=\"resetConfig\" class=\"btn btn-primary\">Reset Config</button>";
	groupContent += "</div>";
	groupContent += "</div>";
...
void WrapperWebconfig::getIdleModes(uint8_t active, LinkedList<SelectEntryBase *> *target) {
	...
	target->add((SelectEntryBase *) new SelectEntry<uint8_t>("Hyperion", "Hyperion UDP", active == HYPERION_UDP,
															 HYPERION_UDP));
	target->add((SelectEntryBase *) new SelectEntry<uint8_t>("Static", "Static color", active == STATIC_COLOR,
															 STATIC_COLOR));
...
void WrapperWebconfig::getLedType(uint8_t active, LinkedList<SelectEntryBase *> *target) {
	for (int i = 0; i < nbLedType; i++)
		target->add((SelectEntryBase *) new SelectEntry<uint8_t>(LedTypeName[i], LedTypeName[i], active == i, i));
}